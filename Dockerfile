...
# Pre-build ChromaDB during image build (critical for cold starts)
RUN python -m app.rag.embed_knowledge

# Hugging Face Spaces strictly requires running as a user with UID 1000
RUN useradd -m -u 1000 user && \
    chown -R user:user /app/chroma_db /app/mlruns /app/knowledge
USER user
